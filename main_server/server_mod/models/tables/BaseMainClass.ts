...
class BaseMainClass <T extends Models>  {
    ...
    constructor(public obj?:T,public fconfig?:Unique|null,public fmodelName?:ModelNames,public attr:any[]=[],public loadedModel?:boolean){////?
        ...
        if(Object.keys(obj||{}).length){           
            confingD[fmodelName].fields.forEach((elem:{[prop:string]:"string"|"number"})=>{
    ...
    async load(){
        ...
        if(this.loadedModel && typeof(this.obj)==="object" && this.obj){
          async function define(number:number,elem:d,resolve:Function){
            ...
            if(confingD[this.fmodelName].has){
                return  fun.call( this,confingD[this.fmodelName].has,1);
            }else if(confingD[this.fmodelName].otherFields){
                return  fun.call( this,confingD[this.fmodelName].otherFields,2);
            }
            return this;
        }
    }
    next(obj:object){
        this.fdiff=Object.fromEntries(Object.entries(obj).filter(([key])=>{///other fields in database
            const obj=confingD[this.fmodelName].otherFields.find((elem:{key:string})=>key==elem.key) //??
            if(obj) return true;
            return false;
        }));
    }
